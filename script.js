...
// متغيرات فارغة ليتم ملؤها بعد الجلب
let allProducts = []; 
...
// 1. دالة جلب المنتجات من ملف JSON
async function fetchProducts() {
    try {
        const response = await fetch('./products.json'); 
        if (!response.ok) {
            // رسالة تنبيه للمستخدم إذا لم يتم العثور على ملف المنتجات (404)
            throw new Error(`فشل في تحميل ملف المنتجات. الحالة: ${response.status}`);
        }
        allProducts = await response.json();
        currentProducts = [...allProducts]; 
        
        // عند التحميل بنجاح، نبدأ في عرض المنتجات
        applyFilters(); 
    } catch (error) {
        console.error("خطأ في جلب المنتجات:", error);
        productGrid.innerHTML = '<p class="no-results" style="grid-column: 1 / -1; text-align: center; padding: 50px; color: #dc3545; font-size: 1.5em;"><i class="fas fa-exclamation-triangle"></i> تعذر تحميل المنتجات من قاعدة البيانات.</p>';
    }
}


// 2. دالة عرض المنتجات في الواجهة مع الرسالة التوجيهية
...
// 5. معالجة الأحداث (Event Listeners)

searchBar.addEventListener('input', applyFilters);
priceSort.addEventListener('change', applyFilters);
productsPerPageSelect.addEventListener('change', applyFilters);
...
// عند تحميل الصفحة - نبدأ بجلب المنتجات
fetchProducts();